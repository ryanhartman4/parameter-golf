# Based on the April 9, 2026 record by bigbag (1.0810 BPB, 3-seed mean)
# Rebased from the LZMA-compressed submission in records/track_10min_16mb/
# 2026-04-09_SP8192_3LayerRecur_ParResid_QK525_LegalTTT/
#
# Key techniques: SP8192, depth recurrence (L3-5 x2), parallel residuals (L7+),
# skip gates, full Hessian GPTQ with SDClip, score-first TTT, byte-shuffle + Brotli
...
class Hyperparameters:
    ...
    # Parallel residuals: GPT-J style attn+mlp from same input at layer >= this
...
# ─── Validation Data & SentencePiece LUTs ────────────────────────────────────
...
# ─── Data Loading (Shuffled Sequence Loader with Memmap) ─────────────────────
...
class MLP(nn.Module):
    ...
    def forward(self, x):
        # LeakyReLU(0.5)^2: preserves half of negative activations, then squares
...
class Block(nn.Module):
    ...
    def forward(self, x, x0):
        ...
        if self.parallel:
            # GPT-J style: attn and mlp read from same pre-residual input
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        # Depth recurrence: build virtual layer indices by repeating [loop_start..loop_end]
        # With num_loops=2 and layers 3-5: [0,1,2, 3,4,5, 3,4,5, 3,4,5, 6,7,8,9,10]
        # giving 17 virtual layers from 11 physical, split into encoder/decoder halves
    ...
    def forward_logits(self, input_ids):
        ...
        for skip_idx, i in enumerate(dec_iter):
            if skip_idx < self.num_skip_weights and skips:
                scaled_skip = (self.skip_weights[skip_idx].to(dtype=x.dtype)[None, None, :]
                               * skips.pop())
...
# ─── GPTQ Quantization (Full Hessian with SDClip) ────────────────────────────
...
def gptq_quantize_weight(w, H, clip_sigmas=3.0, clip_range=63, block_size=128):
    """SDClip: scale = clip_sigmas * std(row) / clip_range, then block-wise Hessian GPTQ."""
...
def gptq_mixed_quantize(state_dict, hessians, h):
    ...
    for name, tensor in state_dict.items():
        ...
        q, s = gptq_quantize_weight(t, hessians[name],
                                     clip_sigmas=cs, clip_range=2 ** (bits - 1) - 1)
        result[name + '.q'] = q
        result[name + '.scale'] = s
...
def _byte_shuffle(data, stride=2):
    """Reorder bytes by position modulo stride to group similar values for compression."""
...
def eval_val_ttt(h, device, val_data, base_model, batch_seqs=32):
    """Score-first TTT: score each chunk under no_grad, THEN train on already-scored tokens."""
    rank = h.rank
    ...
    for ci in range(num_chunks):
        ...
        # Phase 1: Score this chunk (no grad)
        base_model.eval()
        with torch.no_grad():
            for bi in range(0, len(my_windows), batch_seqs):
                batch_ws = my_windows[bi:bi + batch_seqs]
        ...
        # Phase 2: Train on this chunk (skip last chunk)
...
def train_model(h, device, val_data):
    ...
    # Two-phase warmup: first without looping, then with looping, then reset state.
    # This lets the model see both execution paths before real training begins.
    if h.warmup_steps > 0:
        initial_model_state = {
    ...
    while True:
        ...
        if last_step:
            if stop_after_step is not None and step < h.iterations:
                log(f"stopping_early: wallclock_cap train_time: {training_time_ms:.0f}ms "
        ...
        # Activate depth recurrence partway through training
        ...
        should_log_train = (h.train_log_every > 0
