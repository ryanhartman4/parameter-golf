#!/usr/bin/env python3
"""Standalone sliding-window eval with stride=32 (and stride=64 for comparison).

Usage: uv run experiments/eval_stride32.py [checkpoint_path]
...
def main() -> None:
    ...
    # --- Stride=32 eval ---
    print("Running eval with stride=32...")
    torch.cuda.synchronize()
    t0 = time.perf_counter()
    s32_loss, s32_bpb = eval_val_sliding(
        args, model, device, val_tokens,
        base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        stride=32, eval_seq_len=eval_seq_len, compiled_logits=compiled_logits,
    )
    torch.cuda.synchronize()
    s32_time = time.perf_counter() - t0
    print(f"  stride=32: loss={s32_loss:.6f} bpb={s32_bpb:.6f} time={s32_time:.1f}s")

    # --- Stride=64 eval (comparison) ---
    print("Running eval with stride=64...")
    torch.cuda.synchronize()
    t0 = time.perf_counter()
    s64_loss, s64_bpb = eval_val_sliding(
        args, model, device, val_tokens,
        base_bytes_lut, has_leading_space_lut, is_boundary_token_lut,
        stride=64, eval_seq_len=eval_seq_len, compiled_logits=compiled_logits,
    )
    torch.cuda.synchronize()
    s64_time = time.perf_counter() - t0
    print(f"  stride=64: loss={s64_loss:.6f} bpb={s64_bpb:.6f} time={s64_time:.1f}s")

    delta = s32_bpb - s64_bpb
    total_time = s32_time + s64_time

    result = {
        "experiment": "eval_stride32",
        "val_bpb": s32_bpb,
        "val_loss": s32_loss,
        "sliding_bpb": s32_bpb,
        "artifact_bytes": None,
        ...
        "notes": f"stride32={s32_bpb:.4f} stride64={s64_bpb:.4f} delta={delta:.4f}",
    }

    out_path = results_dir / "eval_stride32.json"
